...
namespace SplineMe.Editor
{
	public partial class BezierSplineEditor : UnityEditor.Editor
	{
		...
		private void UpdateNewDrawCurvePainterPosition(Vector3 newEndPosition)
		{
			...
			var firstPointDistance = BazierSplineEditor_Consts.DrawCurveFirstControlPoint * BazierSplineEditor_Consts.DrawCurveSegmentLength;
			...
			if (distance < secondPointDistance && distance >= firstPointDistance && !firstControlPointSet)
			{
				...
				newCurvePoints[1] = Vector3.Lerp(newCurvePoints[1], newEndPosition, Mathf.Abs(BazierSplineEditor_Consts.DrawCurveFirstControlPoint - prevFirstPointNormalizedDistance) / normalizedDistancesDiff);
				firstControlPointSet = true;
			}
		...
		private void SpawnDrawCurveModeCurve()
		{
			var p0 = newCurvePoints[0];
			var p3 = newCurvePoints[3];
			currentSpline.GetInverseControlPoints(p0, p3, newCurvePoints[1], newCurvePoints[2], BazierSplineEditor_Consts.DrawCurveFirstControlPoint, BazierSplineEditor_Consts.DrawCurveSecondControlPoint, out var p1, out var p2);
