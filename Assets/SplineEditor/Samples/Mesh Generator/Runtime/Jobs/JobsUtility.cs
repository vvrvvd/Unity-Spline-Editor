...
{
	public static void Schedule<T>(this T job, int arrayLength, int interloopBatchCount, Action<T> OnJobCompleted, JobHandle dependsOn = default) where T : struct, IDisposableJobParallelFor
	{
...
#if UNITY_EDITOR
	public static EditorCoroutine ScheduleEditorAsync<T>(this T job, int arrayLength, int interloopBatchCount, Object context, Action<T> OnJobCompleted = null, JobHandle dependsOn = default) where T : struct, IDisposableJobParallelFor
	{
		var handle = job.Schedule(arrayLength, interloopBatchCount, dependsOn);
